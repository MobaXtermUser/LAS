...
dracut  -f  && grub2-mkconfig -o /boot/grub2/grub.cfg && grub2-mkconfig -o /boot/efi/EFI/rocky/grub.cfg
```
...
rpm --import http://developer.download.nvidia.com/compute/cuda/repos/rhel9/x86_64/D42D0685.pub
...
sed -i 's/gpgcheck=1/gpgcheck=0/' /etc/yum.repos.d/nodesource-nodejs.repo
...
cat << EOF >> /etc/systemd/system/vivaldiframeworkd.service
[Unit]
Description=Vivaldi Framework Service
After=network.target

[Service]
Type=forking
ExecStart=/etc/init.d/vivaldiframeworkd start
ExecStop=/etc/init.d/vivaldiframeworkd stop
ExecReload=/etc/init.d/vivaldiframeworkd restart
Environment="MSM_HOME=/usr/local/MegaRAID Storage Manager" "PRODUCTNAME=MegaRAID Storage Manager" "MSM_PRODUCT=MSM"
